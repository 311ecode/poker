...
//
// POKER-013: every room this suite creates is marked `test: true` and deleted in
// a `finally`. It must NEVER call `/resetdata` against this semi-live system —
// that would wipe real rooms too.
...
/** Create a TEST room on the origin and remember it for the after-test sweep. */
async function liveTestRoom(
  request: APIRequestContext,
  prefix: string,
  created: string[],
): Promise<string> {
  const response = await request.post(`${ORIGIN}/api/rooms`, {
    data: { title: uniqueTitle(prefix), public: true, test: true },
  });
  expect(response.status(), "POST /api/rooms").toBe(200);
  const code = ((await response.json()) as { room: { code: string } }).room.code;
  expect(code).toMatch(/^[A-Z0-9]{6}$/);
  created.push(code);
  return code;
}

/** POKER-013: remove the rooms this suite made. Never `/resetdata`. */
async function sweepLiveTestRooms(request: APIRequestContext, created: string[]): Promise<void> {
  for (const code of created) {
    try {
      await request.delete(`${ORIGIN}/api/rooms/${code}`, {
        headers: { "x-poker-confirm": "delete" },
      });
    } catch {
      /* the sweep must never fail the suite */
    }
  }
}
...
test.describe("live smoke against the deployed origin", () => {
  ...
  test("health + create-room + name claim", async ({ browser, request }) => {
    ...
    try {
      ...
      const code = await liveTestRoom(request, "live-smoke", created);
      ...
      await sweepLiveTestRooms(request, created);
    }
  });
  ...
  test("POKER-002 live: fixed deck, name gate and visible own vote", async ({ browser, request }) => {
    ...
    try {
      const code = await liveTestRoom(request, "live-deck", created);
  ...
  test("POKER-003 live: the claimed name is reused in another room", async ({ browser, request }) => {
    ...
    const context = await browser.newContext();
    try {
      const roomA = await liveTestRoom(request, "live-name-a", created);
      const roomB = await liveTestRoom(request, "live-name-b", created);
