...
- [ ] AC1 — `vote_open` with a missing, empty or whitespace-only title creates a vote titled
  `Vote <n>` with id `v<n>`, incrementing per room (1, 2, 3 …).
- [ ] AC2 — A typed title is used verbatim; 81 characters is still `bad_title`; a non-string title
  is still `bad_title`.
- [ ] AC3 — The room's Open-a-vote form tells the user they may leave the question empty
  (`placeholder` + hint).
